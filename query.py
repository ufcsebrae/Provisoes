queries = { 
    ...
    WHERE CODTMV = '1.1.99' AND YEAR(DATAMOVIMENTO) BETWEEN 2020 AND 2026)

    ,FINAL AS(
    SELECT A.*,B.IDMOVDESTINO as 'IDMOV_1223',C.DATASAIDA AS 'DATA_223' FROM _NOVENTAENOVE A
    LEFT JOIN CORPORERM.TMOVRELAC B
    ON A.IDMOV_1198 = B.IDMOVORIGEM
	LEFT JOIN CorporeRM.TMOV C
	ON B.IDMOVDESTINO = C.IDMOV
	)
